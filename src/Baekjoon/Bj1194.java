		...
		while(!que.isEmpty()) {
			...
			for (int i = 0; i < dc.length; i++) {
				...
				if(nr>=0 && nr < N && nc>=0 && nc <M && !visited[nr][nc][key] && map[nr][nc] !='#') {
					...
					}else if(map[nr][nc]-97>=0 && map[nr][nc]-97 <=5) { //열쇠면
						que.offer(new Man(nr,nc,dist+1,key | (1<<(map[nr][nc]-97))));
						visited[nr][nc][key | (1<<(map[nr][nc]-97))] = true;
						
					}else if(map[nr][nc]-65>=0 && map[nr][nc]-65 <=5   ) { //문이면
						if(((key & (1<<(map[nr][nc]-65)))) == (1<<(map[nr][nc]-65))) {
							que.offer(new Man(nr,nc,dist+1,key));
							visited[nr][nc][key]=true;
						}
					}
